...
- `docker run -d redis --name redis-server -p 6379:6379 bind 0.0.0.0` runs the docker image `redis` after creating a container with name `redis-server` and binds the docker port `6379` to port `6379` on the IP address `0.0.0.0` mentioned after `bind`
...
- start an existing container in docker using `docker start redis-server` where `redis-server` is the name of the container created in the first step.
